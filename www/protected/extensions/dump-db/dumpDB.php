...
class dumpDB
{
	...
	private function dumpTable($tableName)
	{
		...
		$q = $db->createCommand('SHOW CREATE TABLE '.$db->quoteTableName($tableName).';')->queryRow();
                ...
                $create_query = array_values(array_filter(array_map('trim', explode(',', $create_query))));
                $create_sql = '';
                foreach ($create_query as $ind => $row) {
                    if ($ind === 0) {
                        $create_sql .= $row."\n";
                    } elseif ($ind < count($create_query) - 2) {
                        $create_sql .= "  $row,\n";
                    } elseif ($ind === count($create_query) - 2) {
                        $create_sql .= "  $row\n";
                    } else {
