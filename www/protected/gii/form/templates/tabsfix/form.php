 ...
 */
    ...
    $controllerName = explode('/', $this->viewName);
    if (count($controllerName) > 1) {
        $controllerName = trim(reset($controllerName));
    } else {
        $controllerName = lcfirst($this->getModelClass());
    }
?>
<?php echo "<?php\n"; ?>
/* @var $this <?php echo ucfirst($controllerName) ?>Controller */
...
<?php foreach($this->getModelAttributes() as $attribute): ?>
    <div class="row">
        ...
        <?php echo "<?php if (\$model->hasErrors('$attribute')): ?>\n"; ?>
